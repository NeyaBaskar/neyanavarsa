...
(function setupMudraIllustration() {
  ...
  const formatTitleCaseHyphen = (value) =>
    value
      .split(/[^a-z0-9-]+/i)
      .filter(Boolean)
      .map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase())
  ...
  const addCandidate = (value) => {
    ...
    const normalized = value
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "");
    const compactTitle = formatTitleCase(value);
    const hyphenTitle = formatTitleCaseHyphen(value);
    const rawNoSpaces = value.replace(/\s+/g, "");
    const raw = value.trim();

    [normalized, compactTitle, hyphenTitle, rawNoSpaces, raw].forEach((candidate) => {
      if (candidate) {
